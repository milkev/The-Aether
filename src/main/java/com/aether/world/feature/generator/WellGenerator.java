...
public class WellGenerator {
    private static final ResourceLocation WELL = Aether.locate("well");
    ...
    public static class Piece extends TemplateStructurePiece {
        ...
        public Piece(ServerLevel world, CompoundTag nbt) {
            super(AetherFeatures.WELL_PIECE, nbt, world, (identifier) -> createPlacementData(Rotation.valueOf(nbt.getString("Rot"))));
        }
        ...
        public boolean postProcess(WorldGenLevel world, StructureFeatureManager structureAccessor, ChunkGenerator chunkGenerator, Random random, BoundingBox boundingBox, ChunkPos chunkPos, BlockPos pos) {
            if (this.templatePosition.getY() > 2) {
                if (!shifted) {
                    this.templatePosition = this.templatePosition.below(3);
                    shifted = true;
                }
